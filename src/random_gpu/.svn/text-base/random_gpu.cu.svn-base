...
void gpu_random_init(int* seed,double* ra,int narrays)
{
	if (random_array_index == -1)
	{
		...
		random_gpu_array(random_array_pointer,nnumbers,narrays,seed,1);
...
void gpu_random_clean(void)
{
	cudaFree(gpu_seed);
	cudaFree(gpu_ra_d);
}

void gpu_random_rebuild(int narrays)
{
	int size1 = sizeof(double)*100*narrays;
	cudaMemset(gpu_ra_d,0,size1);

	// Set Kernel Execution Parameters
	// total number of threads = GridDim*BlockDim
	int BlockDim = 256;
	int GridDim = (narrays+BlockDim-1)/BlockDim;

	// Execute GPU routine
	//printf("Before Kernel \n");
	random_gpu_kernel<<<GridDim,BlockDim>>>(gpu_ra_d,
									 gpu_seed_d,100,narrays);

	return;
}

double* get_gpu_random_arrayptr(void)
{
	return gpu_ra_d;
}
